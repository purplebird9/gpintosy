...
(告诉 CPU 跳转到 BIOS 的真正入口地址去执行初始化程序。)
...
>>The call stack shows that palloc_get_page was called by paging_init (frame #1), which in turn was called by pintos_init (frame #2), originating from the boot-time startup code in start (frame #3).
>> 
